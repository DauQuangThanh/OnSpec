...
1) Phase 1 — Dry run (no publish)
   ...
   - Add a repository guard (`if: github.repository == 'DauQuangThanh/OnSpec'`) and a concurrency group for safety.
...
- Repository/branch guard: run publish logic only on upstream `main` (`if: github.repository == 'DauQuangThanh/OnSpec' && github.ref == 'refs/heads/main'`).
